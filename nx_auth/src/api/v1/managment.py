...
async def delete_user_role(
    ...
    if not (
        role := await management_service.get_role_info_by_id(
            role_id=params.role_id, db=db
        )
    ):
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND, detail="Role not found.")

    try:
        payload = {"roles": user.roles.remove(role.title)}
        ...
        raise HTTPException(
            status_code=HTTPStatus.NOT_FOUND, detail="Current role not in verify."
        )
...
async def add_role_to_user(
    ...
    try:
        payload = {"roles": user.roles.append(role.title)}
        access_token = await management_service.generate_new_access(user.token, payload)
        await management_service.add_role_to_user(
            data_to_add=params.model_dump(), db=db
        )
        return {"access_token": access_token}
...
async def get_all_roles(
    ...
    if not result:
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND, detail="No roles found.")
