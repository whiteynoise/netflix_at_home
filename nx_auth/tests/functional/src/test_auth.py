...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"username": "yamle", "email": "yamle@google.com", "password": "wrongpass"},
            422,
            False,
        ),
        (
            {"username": "wrong", "email": "wrong@example.com", "password": "wrong"},
            404,
            False,
        ),
    ],
...
@pytest.mark.parametrize(
    "user_data, expected_status",
    [
        ({"email": "yamle@google.com", "password": "1234567910"}, 200),
        (
            {"email": "yamle@google.com", "password": "wrongpass"},
            422,
        ),
        ({"email": "wrong@wrong123.com", "password": "wrong"}, 404),
    ],
)
@pytest.mark.asyncio
async def test_extra_login_success(
    db_session_with_data, make_post_request, user_data, expected_status
):
...
async def test_change_user_info(
    ...
    response = await make_patch_request(
        "/change_user/1", {"username": "newusername"}, headers=headers
    )
    assert response["status"] == expected_status
    assert ("access_token" in response["body"]) == access_in_body
